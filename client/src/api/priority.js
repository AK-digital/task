...
export async function getPriorityByProject(projectId) {
  try {
    ...
    if (!projectId || projectId === 'undefined' || projectId === 'null' || String(projectId).trim() === '') {
      console.error("getPriorityByProject: Invalid projectId:", { projectId, type: typeof projectId });
    ...
    if (!cleanProjectId || cleanProjectId.length < 7) {
      console.error("getPriorityByProject: Invalid cleaned projectId:", { original: projectId, cleaned: cleanProjectId });
      return []; // Retourner un tableau vide au lieu de lancer une erreur
    }
