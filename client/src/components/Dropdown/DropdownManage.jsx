    ...
    <>
      <div className="relative text-center text-text-color-muted select-none data-[active=true]:z-9999 !gap-0">
        <button
          ...
          className="relative flex justify-center items-center gap-[5px] h-8 bg-color-medium-color rounded-sm text-small w-25"
        >
          <Settings size={16} />
          <span>Gérer</span>
        </button>
        {isOpen && (
          <div className="absolute z-9999 top-10 left-0 w-full bg-secondary text-small rounded-sm max-h-[200px] overflow-auto text-left shadow-medium">
            <ul className="flex justify-center items-center flex-col gap-0 w-full">
                ...
                {canDelete && (
                  <form action={formAction} className="w-full">
                    ...
                    <button
                      ...
                      disabled={pending}
                      className="flex justify-start items-center gap-[15px] text-text-color-muted py-2 px-[5px] bg-transparent rounded-none w-full hover:bg-third group-hover:text-text-dark-color"
                    >
                      <X
                        size={16}
