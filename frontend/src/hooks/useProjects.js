...
function useProjects() {
	...
	const fetchProjects = useCallback(async () => {
		...
		try {
			...
			if (currentUser.currentProjectId) {
				const currentProject = combinedProjects.find(
	...
	const handleUpdateProjectTitle = async (projectId, newTitle) => {
		if (currentProject && currentProject.id === projectId) {
			try {
				...
				console.error(
					"Erreur lors de la mise à jour du titre du projet:",
					error
				);
			}
		}
	...
	const handleDeleteBoard = async (projectId, boardId) => {
		if (currentProject) {
			try {
				const updatedBoards = currentProject.boards.filter(
				...
				if (
					editingTask &&
					!updatedBoards.some((board) =>
						board.tasks.some((task) => task.id === editingTask.id)
					)
				) {
					setEditingTask(null);
				}
	...
	const handleUpdateBoardTitle = async (projectId, boardId, newTitle) => {
		try {
			...
			console.error(
				"Erreur lors de la mise à jour du titre du tableau:",
				error
			);
		}
	};
				...
				if (previousTask.assignedTo !== updatedTask.assignedTo) {
					...
					if (assignedUser) {
						...
						await api.post("/notifications", {
							...
							title: "Tâche",
							task: "Vous avez été assigné à une tâche",
	...
	const handleMoveTask = async (
		...
		if (currentProject && currentProject.id === projectId) {
			...
			const sourceBoard = updatedProject.boards.find(
				(b) => b.id.toString() === sourceBoardId
			);
			const destBoard = updatedProject.boards.find(
				(b) => b.id.toString() === destBoardId
			);
	...
	const handleUploadFiles = useCallback(
		async ({ files, currentUser, currentProject, boardId, taskId }) => {
			if (!files || files.length === 0) {
	...
	const handleInviteUser = async (
		...
		try {
			...
			await api.post("/notify-project-assignment", {
			...
			await api.post("/notifications", {
				...
				description: `Vous avez été ajouté à "${currentProject?.name}"`,
				date: new Date(),
				seen: false,
			});
	...
	const handleUpdateBoardTitleColor = async (projectId, boardId, newColor) => {
		try {
			...
			console.error(
				"Erreur lors de la mise à jour de la couleur du tableau:",
				error
			);
		}
	};
	...
	const handleEditResponse = async (taskId, responseId, updatedResponse) => {
		...
		try {
			...
			setProjects((prev) =>
				prev.map((p) => (p.id === updatedProject.id ? data : p))
			);
			setCurrentProject(data);
		...
		if (currentProject) {
			try {
				...
				updatedProject.boards.forEach((board) => {
					board.tasks.forEach((task) => {
						if (task.id === taskId) {
							task.responses = task.responses.filter(
				...
				setEditingTask((prevTask) => ({
					...prevTask,
					responses: prevTask.responses.filter(
						(response) => response.id !== responseId
					),
				}));
			} catch (error) {
	...
	const handleSendDescriptionMention = async (mentionnedUsers, taskDetails) => {
		try {
			const response = await api.post("/notify-description-mention", {
	...
	const handleSendResponseMention = async (mentionnedUsers, taskDetails) => {
		try {
			const response = await api.post("/notify-response-mention", {
				mentionnedUsers: mentionnedUsers,
				taskDetails: taskDetails,
			});
			...
			for (const mentionnedUser of mentionnedUsers) {
				await api.post("/notifications", {
					...
					title: "Réponse",
					task: taskDetails?.text,
					description: `"${taskDetails?.response}"`,
					date: new Date(),
					seen: false,
				});
			}
	...
	const addUserToSeenTask = async (projectId, taskId, userId) => {
		try {
			const body = {
				taskId: taskId,
				userId: userId,
			...
			const response = await api.patch(
				`/projects/add-task-seen/${projectId}`,
				body
			);
	...
	const addUserToSeenResponse = async (
		projectId,
		userId,
		taskId,
		responseId
		...
		try {
			...
			const response = await api.patch(
				`/projects/add-response-seen/${projectId}`,
				body
			);
