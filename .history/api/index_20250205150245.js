...
io.on("connection", (socket) => {
  ...
  socket.on("create notification", async (sender, receiver, message) => {
    const receiverUser = await UserModel.findOne({ email: receiver?.email });

    if (!    const receiverUser = await UserModel.findOne({ email: receiver?.email });
) {
      throw new Error(
