...
export async function acceptProjectInvitation(req, res, next) {
  try {
    ...
    const user = await userModel.findOne({email: updatedProjectInvitation.guestEmail})

    if(!user) {
       return res.status(404).send({
        success: false,
        message: "Impossible d'accepter l'invitation d'un utilisateur qui n'existe pas",
      });
    }
