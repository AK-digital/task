...
export async function addResponsible(req, res, next) {
  try {
    ...
    const responsible = await userModel.findById({ _id: req.params.id });
    ...
    const subjet = `Vous avez été nommé responsable d'une tâche : ${updatedTask?.text}`;
    const text = `
        <div style="font-family: Arial, sans-serif; color: #333; text-align: center; padding: 20px;">
          <h1 style="color: #5056C8;">Une tâche vous a été assigné sur Täsk</h1>
          <p>Bonjour,</p>
          <p>Vous avez été nommé responsable d'une tâche sur <strong>${updatedTask?.projectId?.name}</strong>. 
        </div>
        `;

    await sendEmail("task@akdigital.fr", email, subjet, text);
