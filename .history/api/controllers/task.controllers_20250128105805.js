...
export async function updateTaskDescription(req, res, next) {
  try {
    ...
    const imgRegex = /<img.*?src=["'](.*?)["']/g; // Ajouter le flag 'g'
    const src = imgRegex.exec(description);

    for (const images of src) {
      const res = await uploadFile("task/description", images);
