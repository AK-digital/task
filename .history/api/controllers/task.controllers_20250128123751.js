...
export async function addResponsible(req, res, next) {
  try {
    ...
    const subjet = `Vous avez été nommé responsable d'une tâche`;
    const text = `
        <div style="font-family: Arial, sans-serif; color: #333; text-align: center; padding: 20px;">
          <h1 style="color: #5056C8;">Une tâche vous a été assigné sur Täsk</h1>
          <p>Bonjour,</p>
          <p>Vous avez été nommé responsable de la tâche : <strong>${updatedTask?.text}</strong> sur <strong>${updatedTask?.projectId?.name}</strong>. 
        </div>
        `;
