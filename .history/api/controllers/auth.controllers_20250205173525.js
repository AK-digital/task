...
export async function sendResetCode(req, res, next) {
  try {
    ...
    const link = `${process.env.CLIENT_URL}/forgot-password/${savedPasswordResetCode?._id}`;
...
export async function resetForgotPassword(req, res, next) {
  try {
    const { resetCode, password, newPassword } = req.body;

    if (!resetCode || !password || !newPassword) {
      return res.status(400).send({
        success: false,
        message: "Paramètres manquants",
      });
    ...
    const resetCodeFound = await PasswordResetCodeModel.findOne({resetCode: resetCode}); })
    ...
    const match = bcrypt.compare(password, newPassword);
  } catch (err) {
    return res.status(500).send({
      success: false,
      message: err.message || "Une erreur inattendue est survenue",
