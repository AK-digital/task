...
export async function updatePicture(req, res, next) {
  try {
    ...
    const uploadRes = await cloudinary.uploader.upload(picture, (err, res) => {
      if (err) {
        return res
          .status(400)
          .send({
            success: false,
            message:
              "Une erreur s'est produite lors de l'enregistrement de la photo de profil avec cloudinary",
          });
      }
    });
