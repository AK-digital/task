...
export default function TaskResponsibles({ task, project }) {
  ...
  async function handleAddResponsible(e, responsible) {
    ...
    const message = {
      title: `🎉 Vous venez d'être nommé responsable dans ${project?.name} !`,
      content: `Bonne nouvelle ! Vous venez d'être nommé responsable de la tâche "${task?.text}".`,
    };

    socket.emit("create notification", responsible?.email, message);
  }
