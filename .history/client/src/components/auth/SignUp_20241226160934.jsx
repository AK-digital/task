...
export default function SignUp({ setSignIn, setSignUp }) {
    ...
    return (
        <div className={styles.container}>
            ...
            <form className={styles.form} action={formAction} onSubmit={(e) => e.preventDefault()}>
                ...
                {state?.errors?.password && (
                    <i>{state?.errors?.password}</i>
                )}
                {/* Buttons */}
                <div className={styles.buttons}>
                    <button data-disabled={pending} className={instrumentSans.className} disabled={pending}>{pending ? "Inscription en cours..." : "S'inscrire"}</button>
