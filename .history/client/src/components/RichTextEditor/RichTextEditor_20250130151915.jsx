...
export default function RichTextEditor({
  ...
  const [mentionsMap, setMentionsMap] = useState(new Map()); // Pour suivre les mentions et leurs positions
  ...
  useEffect(() => {
    if (quillRef.current) {
      ...
      // Fonction pour mettre à jour les mentions après chaque changement
      const updateMentions = () => {
        const text = editor.getText();
        const newMentionsMap = new Map();
        const newTaggedUsers = new Set();

        // Rechercher toutes les mentions dans le texte
        const regex = /@(\w+ \w+)/g;
        let match;
        ...
        while ((match = regex.exec(text)) !== null) {
          ...
          // Trouver l'utilisateur correspondant au nom complet
          const user = project?.guests?.find(
            (guest) => `${guest.firstName} ${guest.lastName}` === fullName
          );

          if (user) {
            newMentionsMap.set(position, user._id);
            newTaggedUsers.add(user._id);
          }
        }

        setMentionsMap(newMentionsMap);
        setTaggedUsers(Array.from(newTaggedUsers));
      };
