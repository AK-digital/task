...
const Span = Quill.import("blots/inline");

class CustomSpan extends Span {
  static create(value) {
    const node = super.create(value);
    node.setAttribute("style", "background-color:#f4c20d; cursor: pointer");
...
CustomSpan.blotName = "mention";
CustomSpan.tagName = "span";

if (typeof window !== "undefined") {
  Quill.register("formats/mention", CustomSpan);
}
      ...
      bindings: {
        mention: {
          ...
          handler: (range) => {
            ...
            const selection = editor.getSelection();
            if (selection) {
              setLastAtPosition(selection.index);
              setIsTaggedUsers(true);
              getMentionPosition();
            }
          },
