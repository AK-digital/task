...
export default function RichTextEditor({
  ...
  function handleKeydown(e) {
    ...
    // Ajout de la récupération du mot avant le caret lors de la frappe
    if (quillRef.current) {
      const editor = quillRef.current.getEditor();
      const selection = editor.getSelection();
      if (selection) {
        const wordBeforeCaret = getWordBeforeCaret(editor, selection.index);
        console.log("Mot avant le caret : ", wordBeforeCaret);
      }
    }
  }
  ...
  // Fonction pour récupérer le mot avant le caret
  function getWordBeforeCaret(editor, caretPosition) {
    // Récupérer le texte avant la position du caret
    const textBeforeCaret = editor.getText(0, caretPosition);
