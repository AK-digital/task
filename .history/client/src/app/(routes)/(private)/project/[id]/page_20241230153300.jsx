  ...
  const tasks = boards?.length
    ? await Promise.all(
        boards.map(async (board) => {
          const boardTasks = await getTasks(id, board.id); // Appelle l'API pour chaque boardId
          return { boardId: board.id, tasks: boardTasks }; // Associe les tâches à chaque boardId
        })
      )
    : [];
